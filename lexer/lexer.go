...
// moves the poistion of the char "up-one"
...
// returns what the next token is
func (l *Lexer) NextToken() token.Token {
	...
	case '[':
		tok = newToken(token.LSB, l.ch)
	case ']':
		tok = newToken(token.RSB, l.ch)
	case ':':
...
// returns the string that's the current token
